...
from .feature_extraction import _calc_wavelets, calc_wavelets, _extract_label
...
# === Feature Paths ===
base_feature_path = f"D:/mine/Shsh/HIAST/projects/grad project/codes/LCNN-master/my_model/features/{feature_type}/{input_type}/{wavelet_type}"

x_train_path = os.path.join(base_feature_path, f"x_train_{feature_type}.h5")
y_train_path = os.path.join(base_feature_path, f"y_train_{feature_type}.h5")
x_val_path = os.path.join(base_feature_path, f"x_val_{feature_type}.h5")
y_val_path = os.path.join(base_feature_path, f"y_val_{feature_type}.h5")

if all(map(os.path.exists, [x_train_path, y_train_path, x_val_path, y_val_path])):
    x_train = load_h5(x_train_path)
    y_train = load_h5(y_train_path)
    x_val = load_h5(x_val_path)
    y_val = load_h5(y_val_path)
else:
    print("Extracting WPT features...")
    df_tr = pd.read_csv(protocol_tr)
    x_train, y_train = calc_wavelets(df_tr, path_tr)
    df_dev = pd.read_csv(protocol_dev)
    x_val, y_val = calc_wavelets(df_dev, path_dev)

    os.makedirs(base_feature_path, exist_ok=True)
    save_h5(x_train_path, x_train)
    save_h5(y_train_path, y_train)
    save_h5(x_val_path, x_val)
    save_h5(y_val_path, y_val)

# === Weights Directory ===
weights_dir = f"D:/mine/Shsh/HIAST/projects/grad project/codes/LCNN-master/my_model/weights/{feature_type}/{input_type}/{wavelet_type}"
os.makedirs(weights_dir, exist_ok=True)

# === Resume from latest weights if available ===
latest_epoch = 0
...
if initial_epoch < epochs:
    print(f"▶️ Training from epoch {initial_epoch + 1} to {epochs}")
    history = lcnn.fit(
        x_train, y_train,
...
def eval_generator(protocol_df, path, batch_size):
    ...
    for i in range(0, len(protocol_df), batch_size):
        ...
        batch_data = [_calc_wavelets(os.path.join(path, utt_id + ".flac")) for utt_id in batch_files]
