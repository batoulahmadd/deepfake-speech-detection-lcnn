...
from .feature_extraction import _calc_dwt, calc_dwt, _extract_label 
...
if feature_type == "dwt":
    if all(map(os.path.exists, [x_train_path, x_val_path, y_train_path, y_val_path])):
        ...
        x_train, y_train = calc_dwt(df_tr, path_tr) 
        print("Extracting dev data...")
        df_dev = pd.read_csv(protocol_dev)
        x_val, y_val = calc_dwt(df_dev, path_dev) 
